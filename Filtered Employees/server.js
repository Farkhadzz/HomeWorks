...
function validateInput(req, res, next) {
    ...
    if (department && typeof department !== 'string') {
        return res.status(400).send({ error: 'Invalid department format' });
    }
    next();
}

app.get('/employees', validateInput, (req, res) => {
    const { name, department } = req.query;
    ...
    if (name) {
        filteredEmployees = filteredEmployees.filter(emp => emp.name.toLowerCase().includes(name.toLowerCase()));
    }
